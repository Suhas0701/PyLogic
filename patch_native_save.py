...
class CanvasRenderer:
    ...
    def _on_save(self, e):
        ...
        if self.page.web:
            ...
            # Native Desktop: Open OS Save Dialog
            self.file_picker.save_file(dialog_title="Save Circuit Project", file_name="circuit.json", allowed_extensions=["json"])

    def _on_save_result(self, e: ft.FilePickerResultEvent):
        # Triggered when user selects a folder and clicks "Save" on Mac/Win
        if e.path and self.bridge:
            json_str = self.bridge.export_project()
            try:
                with open(e.path, "w", encoding="utf-8") as f:
                    f.write(json_str)
                self.show_success(f"Project safely saved to: {e.path}")
...
print("✅ Hybrid Save System Implemented! It will now open a Native OS Save Dialog.")